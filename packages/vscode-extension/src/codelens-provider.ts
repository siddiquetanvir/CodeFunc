...
export class CodeFuncLensProvider implements vscode.CodeLensProvider {
  ...
  public getSummary(document: vscode.TextDocument): FileSummary | undefined {
    ...
    const hash =
      computeContentHash(text, document.languageId) +
      (persona ? `:${persona}` : "")
  ...
  async provideCodeLenses(
    ...
    const hash =
      ...
      (persona ? `:${persona}` : "")
    let summary =
      this.activeSummaries.get(hash) || this.cache.get<FileSummary>(hash)
    ...
    const depsText =
      ...
      summary.dependencies[0] !== "None detected"
        ? summary.dependencies.slice(0, 4).join(", ")
        : ""
    const ioText =
      summary.sideEffects.length > 0 &&
      summary.sideEffects[0] !== "None detected"
    ...
    const wrapIntoLines = (
      ...
      for (const word of words) {
        ...
        if (
          (
            currentLine +
            (currentLine.length > 0 && !currentLine.endsWith(" ") ? " " : "") +
            word
          ).length > maxLen
        ) {
          if (currentLine.trim()) {
    ...
    for (let i = 0; i < roleLines.length; i++) {
      lenses.push(
        new vscode.CodeLens(topOfFileRange, {
          ...
          tooltip:
            "CodeFunc File Overview - Click to open full architectural panel",
      ...
      for (const dLine of depLines) {
        lenses.push(
          new vscode.CodeLens(topOfFileRange, {
            ...
            command: "codefunc.openDetailedOverview",
            tooltip:
              "File dependencies - Click to open full architectural panel",
            arguments: [document.uri, summary],
          }),
        )
      }
      ...
      for (const iLine of ioLines) {
        lenses.push(
          new vscode.CodeLens(topOfFileRange, {
            ...
            command: "codefunc.openDetailedOverview",
            tooltip:
              "Detected file, console, and network I/O - Click to open full architectural panel",
            arguments: [document.uri, summary],
          }),
        )
      }
      ...
      for (const mLine of mechLines) {
        lenses.push(
          new vscode.CodeLens(topOfFileRange, {
            ...
            command: "codefunc.openDetailedOverview",
            tooltip:
              "Algorithmic patterns & mechanisms - Click to open full architectural panel",
            arguments: [document.uri, summary],
          }),
        )
      }
