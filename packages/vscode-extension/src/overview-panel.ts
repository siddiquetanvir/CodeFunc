...
export class CodeFuncOverviewPanel {
        ...
        switch (message.command) {
          ...
          case 'copyMarkdown':
            if (message.text) {
              ...
              vscode.window.showInformationMessage('CodeFunc: Architectural overview copied to clipboard!');
            }
            break;
          case 'close':
            this.dispose();
  ...
  private _getHtmlForWebview(
    ...
    const copySvg = `<svg class="octicon" viewBox="0 0 16 16" width="14" height="14" fill="currentColor"><path d="M0 6.75C0 5.784.784 5 1.75 5h1.5a.75.75 0 0 1 0 1.5h-1.5a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-1.5a.75.75 0 0 1 1.5 0v1.5A1.75 1.75 0 0 1 9.25 16h-7.5A1.75 1.75 0 0 1 0 14.25Z"></path><path d="M5 1.75C5 .784 5.784 0 6.75 0h7.5C15.216 0 16 .784 16 1.75v7.5A1.75 1.75 0 0 1 14.25 11h-7.5A1.75 1.75 0 0 1 5 9.25Zm1.75-.25a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-7.5a.25.25 0 0 0-.25-.25Z"></path></svg>`;

    const mdLines = [
      `### ⚡ Architectural Overview: \`${filePath}\``,
      `**Primary Role**: ${role}`,
      desc ? `**Summary**: ${desc}` : '',
...
<body>
  <div class="container">
    ...
    <textarea id="summaryMarkdown" style="display:none;">${rawMarkdown.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</textarea>
    <div class="actions-bar">
      <button class="btn btn-primary" id="copyBtn" onclick="copyMarkdown()">
        ${copySvg} <span id="copyBtnText">Copy Summary</span>
      </button>
  ...
  <script>
    ...
    function copyMarkdown() {
      const textarea = document.getElementById('summaryMarkdown');
      const text = textarea ? textarea.value : '';
      sendMessage('copyMarkdown', { text });
