...
const SUMMARY_JSON_SCHEMA = {
  ...
  properties: {
    coreRole: {
      type: "string",
      description:
        "One concise sentence (max 15 words) explaining what this file accomplishes.",
    },
    detailedSummary: {
      type: "string",
      description:
        "A comprehensive 2-4 sentence architectural overview explaining what this file accomplishes, how data flows, and key algorithms or components.",
    },
    keyMechanisms: {
      type: "array",
      items: { type: "string" },
      description:
        'Key algorithms, data structures, or patterns utilized (e.g. "2D Grid Scan", "Manhattan distance").',
    },
    dependencies: {
      type: "array",
      items: { type: "string" },
    ...
    sideEffects: {
      ...
      items: { type: "string" },
      description:
        "List of files read/written, network endpoints, or database tables invoked.",
    },
  },
...
export function detectProvider(
  ...
  if (specifiedProvider && specifiedProvider !== "auto") {
    if (
      specifiedProvider === "groq" ||
      specifiedProvider === "openrouter" ||
      specifiedProvider === "gemini" ||
      specifiedProvider === "anthropic"
    ) {
  ...
  try {
    ...
    if (resolvedProvider === "groq") {
      summary = await callOpenAICompatible(
        apiKey,
        model && model !== "gemini-2.5-flash" && model !== "gemini-flash-latest"
          ? model
...
function generateSmartLocalSummary(
  ...
  const isDockerfile =
    lowerLang === "dockerfile" ||
    lowerBase.includes("dockerfile") ||
    mechanisms.includes("Docker Container Build")
  const isCompose =
    lowerBase.includes("docker-compose") ||
    lowerBase.includes("compose.yaml") ||
    lowerBase.includes("compose.yml") ||
    mechanisms.includes("Docker Compose Multi-Service")
  const isWorkflow =
    (lowerLang === "yaml" || lowerLang === "yml") &&
    (mechanisms.includes("CI/CD Workflow Pipeline") ||
      (filePath ? filePath.includes(".github/workflows") : false))
  const isShell =
    lowerLang === "shellscript" ||
    lowerLang === "bash" ||
    lowerLang === "sh" ||
    lowerLang === "zsh"
  const isJson = lowerLang === "json" || lowerLang === "jsonc"
  const isSql =
    lowerLang === "sql" || mechanisms.includes("SQL Database Operations")

  if (metadata.leadComment) {
    role = metadata.leadComment
    if (
      metadata.signatures.includes("main") &&
      !role.toLowerCase().includes("main")
    ) {
      role += " (main)"
    }
  } else if (isDockerfile) {
    const baseImg = metadata.dependencies[0]
    const stages = metadata.signatures
      .filter((s) => s.startsWith("Stage: "))
      .map((s) => s.replace("Stage: ", ""))
    const entry = metadata.signatures
      .find((s) => s.startsWith("Entry: "))
      ?.replace("Entry: ", "")

    role = baseImg
      ? `Docker container build based on ${baseImg}`
      : "Docker container build specification"
    parts.push(role + ".")
    if (stages.length > 0) {
    ...
    const services = metadata.signatures
      .filter((s) => s.startsWith("Service: "))
    ...
    if (metadata.dependencies.length > 0) {
      parts.push(
        `Spins up images: ${metadata.dependencies.slice(0, 4).join(", ")}.`,
      )
    }
    if (metadata.sideEffects.length > 0) {
      parts.push(
        `Configures ${metadata.sideEffects.slice(0, 3).join(" and ")}.`,
      )
    }
  } else if (isWorkflow) {
    ...
    if (metadata.dependencies.length > 0) {
      parts.push(
        `Coordinates actions: ${metadata.dependencies.slice(0, 4).join(", ")}.`,
      )
    }
  } else if (isShell) {
    ...
    role =
      tools.length > 0
        ? `Shell script coordinating ${tools.join(", ")}`
        : "Shell automation and execution script"
    parts.push(role + ".")
    if (metadata.signatures.length > 0) {
      parts.push(
        `Defines functions: ${metadata.signatures.slice(0, 4).join(", ")}.`,
      )
    }
    if (metadata.sideEffects.length > 0) {
    ...
    if (lowerBase === "package.json") {
      const pkgName = metadata.signatures
        .find((s) => s.startsWith("Package: "))
        ?.replace("Package: ", "")
      role = pkgName
        ? `Package manifest for ${pkgName}`
        : "Node.js package manifest and dependency configuration"
      parts.push(role + ".")
      const scripts = metadata.signatures
      ...
      if (metadata.dependencies.length > 0) {
        parts.push(
          `Declares key dependencies: ${metadata.dependencies.slice(0, 6).join(", ")}.`,
        )
      }
    } else if (lowerBase === "tsconfig.json") {
      role = "TypeScript compiler and project configuration"
      parts.push(
        "Configures TypeScript compilation settings, module resolution, and build targets.",
      )
    } else {
      role = `JSON configuration schema (${baseName || "config"})`
      parts.push(
        `Structured data and environment configuration for ${baseName || "project"}.`,
      )
    }
  } else if (isSql) {
    ...
    role =
      tables.length > 0
        ? `SQL queries and operations on ${tables.join(", ")}`
    ...
    role = baseName
      ? `Module logic for ${baseName}`
      : "Module definitions and program logic"
  }
  ...
  if (parts.length === 0) {
    ...
    if (metadata.signatures.length > 0) {
      const cleanSigs = metadata.signatures.slice(0, 4).map((s) =>
        s
          .replace(/^def\s+/, "")
          .replace(/^function\s+/, "")
          .replace(/\(.*$/, "()"),
    ...
    if (metadata.sideEffects.length > 0) {
      parts.push(
        `Executes operations with ${metadata.sideEffects.slice(0, 2).join(" and ")}.`,
      )
    }
  }
...
function sanitizeSummary(raw: any, metadata: ExtractedMetadata): FileSummary {
  const coreRole =
    typeof raw.coreRole === "string" && raw.coreRole.trim()
      ? raw.coreRole.trim()
      : "Performs module logic and data handling"

  const detailedSummary =
    typeof raw.detailedSummary === "string" && raw.detailedSummary.trim()
      ? raw.detailedSummary.trim()
      : undefined

  const keyMechanisms =
    Array.isArray(raw.keyMechanisms) && raw.keyMechanisms.length > 0
      ? raw.keyMechanisms
      : metadata.patterns
  ...
  const sideEffects =
    Array.isArray(raw.sideEffects) && raw.sideEffects.length > 0
      ? raw.sideEffects
      : metadata.sideEffects
