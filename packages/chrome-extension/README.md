...
Key Mechanisms: Docker Container Build
```
...
- **Intelligent Offline Fallback**: Works completely free without an API key using `@codefunc/core`'s on-device static analysis engine.
...
3. Build the Chrome extension:
   ...
   ```
   *(Or run `npm run watch --workspace=packages/chrome-extension` for auto-rebuilding while making edits)*
